"""Test uer authentication endpoints"""
...
def test_signup_test_admin():
    ...
    result = app.test_client()
    response = result.post('/api/v1/auth/signup', data=json.dumps(mock_reg[11]), content_type='application/json')
    assert (response.status_code ==201) 
...
def test_signin_admin():
    ...
    result = app.test_client()
    old_log_users = bool(logged_user)
    response = result.post('/api/v1/auth/signin', data=json.dumps(mock_log[7]), content_type='application/json')
    json.loads(response.data)
    new_log_users = bool(logged_user)
    print (logged_user)
    assert old_log_users != new_log_users
